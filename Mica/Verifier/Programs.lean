...
private theorem declareAndAssume_correct {primitives : PrimEncodings}
    ...
    ∃ acc' st' ρ', Inv acc' st' ρ' ∧
      ...
      Q acc' st' ρ' := by
  obtain ⟨hacc, howns, hvars, hwf, hΓwf, hΓagree⟩ := hinv
    ...
    | ok info =>
      ...
      obtain ⟨hprimsd, hΓsd, hΔsd, hfnsd, hf, hspec_delta, hspec_fm, hinfoEq⟩ :
        ...
        case neg =>
          ...
          obtain ⟨bv, axs⟩ := tup
          cases hext
          refine ⟨rfl, rfl, rfl, rfl, { symFresh := ?_, argFresh := ?_ }, rfl, rfl, hinfoTuple⟩
          · intro n hn
...
theorem assemble_correct (primitives : PrimEncodings) (hlaw : primitives.Lawful)
  ...
  obtain ⟨acc, st1, ρ1, hinv1, hsub1, hag1, hcont⟩ :=
    assembleFrom_correct hlaw prog { empty with delta := st.decls } st ρ
      ⟨rfl, howns0, hvars0, hwf0, hempty_Γwf, hempty_Γagree⟩
...
def ValDecl.check (reg : Verifier.Registry) (Θ : TinyML.TypeEnv) (Δ_spec : Signature)
  ...
  VerifM.seq
    (do let _ ← compile reg Θ Δ_spec Γfn Gf Bindings.empty B Γ d.body; pure ())
    (pure d.body.ty)
...
def Program.verify (reg : Verifier.Registry) (prog : Untyped.Program Untyped.SpecBody) : Smt.Strategy Smt.Strategy.Outcome :=
  VerifM.strategy do
    ...
    Program.check reg Θ relations.delta relations.functionMap GhostFns.empty Bindings.empty
      TinyML.TyCtx.empty typed
