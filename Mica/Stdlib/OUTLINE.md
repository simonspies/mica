...
- `FixedIntStd.lean` — Native-bitvector `Int32` and `Int64` intrinsics and their soundness instances.
...
- `LogicStd.lean` — Logical primitives for specifications only (`Logic.eq`, `Logic.size`): equality and structural size of values, with precondition `False` so that no program can call them.
