...
let abs_ (x: int) : int = if x < 0 then 0 - x else x + 0
[@@spec fun x ->
  bind (isint x) @@ fun n ->
  ret (fun v ->
    bind (isint v) @@ fun r ->
    if n >= 0 then assert (r = n + 0); ret ()
    else assert (r = -n); ret ())];;

(* ReLU / max-with-zero: exact spec r = max(n, 0). *)
let relu (x: int) : int = if x < 0 then 0 else x + 0
[@@spec fun x ->
  bind (isint x) @@ fun n ->
  ret (fun v ->
    bind (isint v) @@ fun r ->
...
[@@spec fun x ->
  ...
  ret (fun v ->
    ...
    else if n > 10 then assert (r = 10 + 0); ret ()
    else assert (r = n + 0); ret ())];;
